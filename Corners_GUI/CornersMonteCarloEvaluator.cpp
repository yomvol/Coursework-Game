...
CornersMonteCarloEvaluator::CornersMonteCarloEvaluator(CornersBoardModel* board, int numIterations, Tile StartTurn, unsigned int startcol, unsigned int startrow, unsigned int endcol, unsigned int endrow, unsigned int NumWhiteTurns, unsigned int NumBlackTurns)
{
	this->board = board;
...
{
	CornersBoardModel* b = new CornersBoardModel(this->board);
	...
	player1->SetupPlayer(L"RandomWhite", Tile_White);
	player2->SetupPlayer(L"RandomBlack", Tile_Black);
	player1->SetBoard(b);
	player2->SetBoard(b);
	if (DidHop == false)
	{
	...
	while (!GameFinished)
	{
		...
		while (!currentPlayer->MakeMove(0, 0, 0, 0, 0, 0)) //Arguments are passed for compapability and have no meaning
