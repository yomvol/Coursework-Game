...
bool CornersRandomPlayer::MakeMove(unsigned int backwardComp1, unsigned int backwardComp2, unsigned int backwardComp3, unsigned int backwardComp4, unsigned int backwardComp5, unsigned int backwardComp6)
{
	if (DidHop == false)
	{
	...
	while (counter < NumOfHypotheses)
	{
		...
		if (this->board->CheckLegal(startcol, startrow, endcol, endrow))
		{
			...
			int FieldDiff = CornersBoardModel::GetField(endcol, endrow) - CornersBoardModel::GetField(startcol, startrow); // Negative for whites
			// Positive for blacks
			double Correction;
